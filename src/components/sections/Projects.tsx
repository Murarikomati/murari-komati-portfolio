      ...
      <div className="max-w-6xl mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {filteredProjects.map((project) => (
            <Card key={project.id} className="group border-border/50 overflow-hidden hover:border-primary/50 transition-all flex flex-col bg-card shadow-lg">
              <div className="relative aspect-video overflow-hidden">
                ...
                <div className="absolute bottom-4 left-4 z-10">
                  <Badge className="bg-primary text-white font-bold">{project.category}</Badge>
                </div>
              </div>
              <CardHeader>
              ...
              <CardContent className="flex-1">
                ...
                <div className="flex flex-wrap gap-2">
                  {project.tech.map((t) => (
                    <Badge key={t} variant="secondary" className="text-[10px] font-bold tracking-tight py-1 px-3 bg-primary/15 text-primary border-primary/20">
                      {t}
                    </Badge>
                  ))}
                </div>
              </CardContent>
              <CardFooter className="pt-4 flex gap-3 border-t">
                <Button size="sm" variant="ghost" className="flex-1 rounded-lg">
                  <Github className="mr-2 h-4 w-4" /> Code
                </Button>
                <Button size="sm" className="flex-1 rounded-lg shadow-md">
