...
import { Sparkles, Loader2, CheckCircle2, ExternalLink, Github, Linkedin, Code, RefreshCw, Zap, Award } from "lucide-react";
...
// Helper to get the correct icon for links
...
// Sub-component for displaying the results card
function ResultCard({ result, onReset }: { result: MatchSkillsToJobDescriptionOutput; onReset: () => void; }) {
  const {
    impactSummary,
    matchedSkills,
    matchedProjects,
  ...
  return (
    <Card className="shadow-2xl overflow-hidden animate-fade-in-up rounded-[2.5rem]">
      ...
      <CardContent className="p-8 space-y-8 max-h-[650px] overflow-y-auto custom-scrollbar">
        <div className="space-y-2">
          <h4 className="font-bold text-xs uppercase tracking-[0.2em] text-primary">Core Value Proposition</h4>
          <p className="text-base leading-relaxed font-medium text-muted-foreground whitespace-pre-line">{impactSummary}</p>
        </div>

        <div className="space-y-3">
          <h4 className="font-bold text-xs uppercase tracking-[0.2em] text-primary">Matched Engineering Skills</h4>
          <div className="flex flex-wrap gap-2.5">
            {matchedSkills.map((skill, i) => <TechPill key={i} label={skill} />)}
          </div>
        </div>
          ...
          <div className="space-y-4">
            {matchedProjects.map((project, i) => (
              <div key={i} className="bg-muted/50 p-6 rounded-3xl border hover:border-primary/50 transition-all">
                ...
                <p className="text-sm text-muted-foreground leading-relaxed font-medium mb-3">{project.reason}</p>
                <div className="flex items-center gap-2 text-amber-500">
                  <Zap className="h-4 w-4" />
                  <p className="text-sm font-bold">Impact: <span className="font-medium text-muted-foreground">{project.impact}</span></p>
                </div>
              </div>
            ))}
          </div>
        ...
        {matchedCertifications && matchedCertifications.length > 0 && (
          <div className="space-y-4">
            <h4 className="font-bold text-xs uppercase tracking-[0.2em] text-primary">Relevant Certifications</h4>
            <div className="space-y-4">
              {matchedCertifications.map((cert, i) => (
                <div key={i} className="bg-muted/50 flex items-center gap-4 p-4 rounded-3xl border hover:border-primary/50 transition-all">
                  <Award className="h-6 w-6 text-primary" />
                  <div>
                    <p className="font-bold text-base">{cert.name}</p>
                    <p className="text-sm text-muted-foreground font-medium">{cert.relevance}</p>
                  </div>
                </div>
              ))}
            </div>
        ...
        <div className="space-y-3">
          ...
          <div className="grid grid-cols-1 gap-3">
            {recommendedLinks.map((link, i) => (
              <a key={i} href={link.url} target="_blank" rel="noopener noreferrer" className="group flex items-center justify-between p-5 rounded-3xl border bg-muted/30 hover:bg-muted transition-all shadow-sm">
        ...
        <Button variant="ghost" className="w-full rounded-full text-xs text-muted-foreground hover:text-primary hover:bg-primary/10 py-8 font-bold uppercase tracking-widest transition-colors border-dashed border mt-4" onClick={onReset}>
...
// Sub-component for the initial state card
...
export default function SkillMatcher() {
  ...
  const handleMatch = async () => {
    if (!jobDescription.trim() || jobDescription.length < 30) {
      toast({ title: "Input Required", description: "Please paste a job description to start the scan.", variant: "destructive" });
      return;
    }
    ...
    try {
      ...
      toast({ title: "Scan Failed", description: `AI analysis failed: ${error.message}. Please try again.`, variant: "destructive" });
    } finally {
      setIsMatching(false);
    }
  };
        ...
        <div className="flex flex-col lg:flex-row gap-12 items-start">
          <div className="flex-1 space-y-8">
            ...
            <div className="space-y-4">
              ...
              />
              <Button onClick={handleMatch} disabled={isMatching} size="lg" className="w-full h-16 rounded-full text-lg font-bold shadow-2xl shadow-primary/30 hover:scale-[1.01] transition-all bg-primary text-primary-foreground">
                {isMatching ? (
          ...
          <div className="flex-1 w-full lg:sticky lg:top-24 min-h-[600px]">
            {result ? <ResultCard result={result} onReset={handleReset} /> : <PendingCard />}
          </div>
        </div>
      </div>
    </section>
